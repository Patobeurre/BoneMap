...
class ProcessView(Frame):
    ...
    def launchProcess(self):
        ...
        if self.isValidSettings:
            ...
            try:
                #self.process.launch()
                thread = Thread(target = self.launch_thread, args = ())
                thread.start()
    ...
    def launch_thread(self):
        ...
        #p = Pool()
        #thread = p.map(self.process.launch, [])
        #p.close()
        ...
        #result = p.starmap(test_threaded_proc, [
        #    (0, self.nbSectionDone),
        #    (1, self.nbSectionDone),
        #    (2, self.nbSectionDone)
        #    ])
    ...
    def __init__(self, parent, **kwargs):
        ...
        manager = Manager()
        self.nbSectionDone = manager.Value('i', 10)
        ...
        Checkbutton(self, text="Skip preparation", hoverMsg="Skip the reconstruction process", command=self.setSkipPrepare, variable=self.bSkipPrepare).grid(row=1, column=0, sticky='w')
        ...
        self.prepareFrame = LabelFrame(self, text='Prepare serie')
        ...
        progressBarFrame = Frame(self)
        self.progressBar = ProgressbarWithValue(progressBarFrame, value=0, maximum=100, orient="horizontal", length=200, mode="indeterminate", takefocus=True)
        progressBarFrame.grid(row=7, column=0, sticky='news', padx=(0,10))

        Button(self, text="Launch", command=self.launchProcess).grid(row=7, column=1, sticky='se')
